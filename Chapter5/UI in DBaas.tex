...
\subsection{Partially Covered Backend Features}

Several backend capabilities are implemented in the API but not yet represented with dedicated UI screenshots in this chapter:

\begin{itemize}
    \item \textbf{MongoDB workspace:} Collection and document management endpoints exist under project-specific routing paths, but the UI coverage shown here focuses primarily on PostgreSQL workflows (schema editor, SQL query editor, table management).
    \item \textbf{Google OAuth login:} The backend exposes \texttt{/auth/google/login} and callback routes; the Flutter client supports OAuth as an alternative to email-and-password registration.
    \item \textbf{Project access and PostgREST:} External connection strings and PostgREST credentials are available through the project access API; direct-client integration is primarily documented in the backend chapters.
    \item \textbf{Backup export/import:} Streaming backup endpoints are implemented server-side; the UI may expose these through project settings depending on the client build under test.
\end{itemize}

These gaps reflect incremental frontend delivery rather than missing backend functionality. They are noted so readers can distinguish between platform capability (API + operators) and presentation-layer completeness (Flutter screens documented here).